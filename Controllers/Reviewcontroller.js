...
export async function createReview(req, res) {
  try {
    ...
    // ── Verified badge: true only if this user has a confirmed booking
    // referencing this exact package (real signal, not self-reported) ──────
    let verified = false;
    if (req.user?._id) {
      const confirmedBooking = await Booking.findOne({
        user: req.user._id,
        status: 'confirmed',
        $or: [{ packageId: pkgId }, { packageName: pkg.title }],
      });
      verified = Boolean(confirmedBooking);
    }
